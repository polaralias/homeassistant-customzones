...
If tracker data is unusable, the system should prefer explicit degraded behavior over confidently wrong zone membership.
...
A small codebase becomes hard to maintain when its behavior lives only in one developer's memory.

## 4. Geometry is not a detail here

Polygon math is part of the product, not just a helper. Boundary behavior, degenerate inputs, and coordinate semantics are user-facing concerns.
...
When behavior is important enough to defend publicly, it should exist as a clear written rule before or alongside code.
